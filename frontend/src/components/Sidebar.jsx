...
export default function Sidebar({ collapsed = false, onToggle }) {
  return (
    <aside
      className={`bg-[#f4f7ed] h-screen flex flex-col border-r border-[#e5e7eb] font-sans flex-shrink-0 transition-[width] duration-300 ${
    ...
    >
      ...
      <div
        className={
          collapsed ? 'pt-6 pb-6 flex flex-col items-center gap-4' : 'p-6 pb-8 flex items-center justify-between'
        }
      >
        <h1 className="flex items-center gap-2">
          <span className="text-xl font-bold tracking-tight text-[#111827]">HOB</span>
          {!collapsed && (
            <span className="text-[10px] font-semibold tracking-widest text-[#6b7280] uppercase mt-1">
              HOUSE OF BEAUTY
            </span>
          )}
        </h1>
