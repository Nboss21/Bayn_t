      ...
      <div className="bg-[#f2f4ec] py-20 px-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end w-[95%]">
          ...
          <div className="w-full md:w-[45%] lg:w-[35%]">
             <a href="#" className="text-[#a87f59] text-[15px] font-bold flex items-center border-b border-[#dad9cd] pb-3 hover:opacity-80 transition w-full group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {teachers.map((teacher, index) => (
              <div key={index} className="flex flex-col items-center text-center">
                {/* Image Placeholder */}
                <div className="w-full aspect-[4/5] bg-gray-50 rounded-lg mb-8 border border-gray-100"></div>
                
                <h3 className="text-lg font-bold text-[#1c1c1c] mb-1.5">{teacher.name}</h3>
                <p className="text-gray-500 text-[13px] mb-4">{teacher.role}</p>
                <a href="#" className="text-[#d4a373] text-[13px] font-medium border-b border-[#d4a373] pb-0.5 hover:opacity-80 transition">
                  View Profile
                </a>
              </div>
            ))}
