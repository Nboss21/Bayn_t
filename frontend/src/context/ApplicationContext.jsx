...
export function ApplicationProvider({ children }) {
  ...
  useEffect(() => { if (!user) return; applicationService.list({ per_page: 100 }).then((data) => { const drafts = data?.data || data || []; const draft = drafts.find((item) => item.status === 'draft'); if (draft) { setApplication(draft); setFormData((prev) => ({ ...prev, programId: draft.program_id, intakeId: draft.intake_id, applicantPhone: draft.applicant_phone || user.phone || '' })); } else { setFormData((prev) => ({ ...prev, applicantPhone: prev.applicantPhone || user.phone || '' })); } }).catch(() => {}); }, [user]);
  ...
  const ensureDraft = useCallback(async () => { if (application?.id) return application; const draft = await applicationService.create({ program_id: formData.programId, intake_id: formData.intakeId, applicant_name: user?.name, applicant_phone: formData.applicantPhone || user?.phone }); setApplication(draft); return draft; }, [application, formData, user]);
  const saveStep = useCallback(async (step) => { const draft = await ensureDraft(); const payload = { program_id: formData.programId, intake_id: formData.intakeId, applicant_name: user?.name, applicant_phone: formData.applicantPhone || user?.phone }; const saved = await applicationService.saveStep(draft.id, step, payload); setApplication(saved); return saved; }, [ensureDraft, formData, user]);
