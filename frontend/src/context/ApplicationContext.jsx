...
const initialState = { programId: null, intakeId: null, applicantPhone: '', city: '', area: '', landmark: '', education: null, experience: null, idDocument: null, profilePhoto: null, supportingDoc: null, paymentMethod: 'primary', agreed: false };
...
export function ApplicationProvider({ children }) {
  ...
  useEffect(() => { if (!user) return; applicationService.list({ per_page: 100 }).then((data) => { const applications = data?.data || data || []; const draft = applications.find((item) => ['draft', 'rejected'].includes(item.status)); if (draft) { setApplication(draft); setFormData((prev) => ({ ...prev, programId: draft.program_id, intakeId: draft.intake_id, applicantPhone: draft.applicant_phone || user.phone || '', city: draft.city || '', area: draft.area || '', landmark: draft.landmark || '', education: draft.education || null, experience: draft.experience || null })); } else { setFormData((prev) => ({ ...prev, applicantPhone: prev.applicantPhone || user.phone || '' })); } }).catch(() => {}); }, [user]);
  ...
  const applicationPayload = useCallback(() => ({ program_id: formData.programId, intake_id: formData.intakeId, applicant_name: user?.name, applicant_phone: formData.applicantPhone || user?.phone, city: formData.city, area: formData.area, landmark: formData.landmark, education: formData.education, experience: formData.experience }), [formData, user]);
  const ensureDraft = useCallback(async () => { if (application?.id) return application; const draft = await applicationService.create(applicationPayload()); setApplication(draft); return draft; }, [application, applicationPayload]);
  const saveStep = useCallback(async (step) => { const draft = await ensureDraft(); const saved = await applicationService.saveStep(draft.id, step, applicationPayload()); setApplication(saved); return saved; }, [ensureDraft, applicationPayload]);
  ...
  const submitApplication = useCallback(async (draftOverride = null) => {
    const draft = draftOverride || await ensureDraft();
    const submitted = await applicationService.submit(draft.id);
    setApplication(submitted);
    return submitted;
  }, [ensureDraft]);
  return <ApplicationContext.Provider value={{ formData, programs, intakes, application, loading, errors, furthestStep, updateField, getSelectedProgram, validateStep, completeStep, canAccess, setErrors, ensureDraft, saveStep, uploadDocuments, submitApplication }}>{children}</ApplicationContext.Provider>;
}
