...
Backups use the configured MySQL/MariaDB credentials from Laravel's database connection and invoke mysqldump/mysql through a fixed-argument process (no shell interpolation and no hardcoded credentials). Each archive is a timestamped backup-YYYYMMDD-HHMMSS-random.tar.gz file stored beneath the private BACKUP_DISK and BACKUP_DIRECTORY configuration. The archive contains a complete database dump and, when BACKUP_INCLUDE_FILES=true, the configured private document storage and public assets such as certificates, uploaded documents, and gallery images.

Administrative API endpoints are:

* POST /api/backups creates a backup.
* GET /api/backups lists safe metadata only.
* GET /api/backups/{backup} returns one metadata record.
* DELETE /api/backups/{backup} deletes a validated backup.
* POST /api/backups/{backup}/restore restores a backup and first creates a safety backup. Restore is super-admin-only.
* GET /api/backups/{backup}/download is available only through a short-lived signed URL generated by the protected API; the archive is never public.

Artisan commands are backup:create, backup:list, and backup:restore {backup}. Restore requires interactive confirmation. The scheduler creates backups according to BACKUP_SCHEDULE (hourly, daily, or weekly) and prunes archives older than BACKUP_RETENTION_DAYS. Configure the server cron with: * * * * * cd /path/to/backend && php artisan schedule:run >> /dev/null 2>&1.

Only super admins and registrars can create/list/delete backups; only super admins can restore. Backup names are allowlisted and path traversal is rejected. Audit events record creation, deletion, restore start, completion, and failure without storing credentials or dump contents. A successful backup means the archive was created and verified; a successful restore is reported separately only after the database import completes.

## Security hardening

Sanctum bearer tokens expire according to SANCTUM_EXPIRATION, are revoked on logout/password reset/refresh, and sensitive authentication/public-write endpoints are throttled. Resource policies and query scopes enforce administrator, registrar, teacher-assigned-class, and student-ownership boundaries. Program/intake mutations are administrative-only. Private documents and backups require authorization before URL generation; generated URLs are signed and short-lived. Uploads use generated storage names, private storage where appropriate, MIME/extension/size validation, and no client-controlled paths.

Production must use APP_DEBUG=false, a strong private APP_KEY, HTTPS, explicit CORS_ALLOWED_ORIGINS, protected environment files, private backup/document storage, and server-level limits on mysqldump/restore access. Security response headers are added by middleware. Do not place credentials in .env.example or API responses.
